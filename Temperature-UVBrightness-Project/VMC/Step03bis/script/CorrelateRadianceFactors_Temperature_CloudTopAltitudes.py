...
VMCSelectedImages = HandyTools.readTable ('../VMCSelectedImages_CloudTopAltitudes.dat')
...
for iImage in range (numberOfVMCImages):
    ...
    #  and the latitude limits. 
    if VMCOrbitID >= VMCOrbitBoundaries [0] and VMCOrbitID <= VMCOrbitBoundaries [1] and \
       phaseAngle <= phaseAngleLimit and\
       numberOfPointInLatLonBox >= numberOfPointInLatitudeLongitudeBoxMinimum and\
       VMCSelectedImages [0][6][iImage] >= latitudeLimits [0] and VMCSelectedImages [0][6][iImage] <= latitudeLimits [1]:
