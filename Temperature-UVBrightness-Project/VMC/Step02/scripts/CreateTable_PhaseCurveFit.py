...
# From the table created by  CreateTable_PhaseCurve.py  in this same Step02 of the process, extract a binned phase curve. 
...
# Select and read the data from the phase curve file.
incidence_emission_angle_String = 'i<84_e<81'
# incidence_emission_angle_String = 'i<89_e<89'

phaseCurveData = HandyTools.readTable ( os.path.join ( VMCWorkBookDirectory, 'Step02', 'PhaseCurve_{}.dat'.format (incidence_emission_angle_String) ) )
...
selectionParameterSets =  [ 
    ...
    [ 'V1000', 130, '../plots_{}/PhaseCurveExtension2-4+SPDC_PhaseAngleLT130dgr.png'.format (incidence_emission_angle_String), 'EXT2-4 + SPDC & phase angles < 130˚', False ], 

    # Setting for the plots with binned data.
    [ 'V0000', 180, '../plots_{}/PhaseCurveBinned_allPhaseAngles.png'.format (incidence_emission_angle_String), 'Whole mission & all phase angles (binned {}˚)'.format (binSize), True ],
    [ 'V0500', 180, '../plots_{}/PhaseCurveBinnedExtension1-4+SPDC_allPhaseAngles.png'.format (incidence_emission_angle_String), 'EXT 1-4 + SPDC & all phase angles (binned {}˚)'.format (binSize), True ],
    [ 'V1000', 180, '../plots_{}/PhaseCurveBinnedExtension2-4+SPDC_allPhaseAngles.png'.format (incidence_emission_angle_String), 'EXT 2-4 + SPDC & all phase angles  (binned {}˚)'.format (binSize), True ],
    [ 'V1000', 130, '../plots_{}/PhaseCurveBinnedExtension2-4+SPDC_PhaseAngleLT130dgr.png'.format (incidence_emission_angle_String), 'EXT2-4 + SPDC & phase angles < 130˚ (binned {}˚)'.format (binSize), True ],
...
fileOpen = open ( os.path.join (VMCWorkBookDirectory, 'Step02', phaseCurveFitFileName), 'w' )

headerLines = [
'',
'  RF (pa)= {:8.6f} * pa^2 + {:7.4f} * pa + {:7.3f}  |  r^2 = {:5.3f} (pa = phase angle in ˚)'.format ( polynomialCoefficients [2], 
...
headerString = HandyTools.getTableHeader (phaseCurveFitFileName, creationScript = 'CreateTable_PhaseCurveFit.py', headerLines = headerLines)
